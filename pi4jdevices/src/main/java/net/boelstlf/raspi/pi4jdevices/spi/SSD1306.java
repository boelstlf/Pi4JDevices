 ...
 */
package net.boelstlf.raspi.pi4jdevices.spi;
...
public class SSD1306 {
     ...
     */
    public SSD1306(int width, int height, GpioController gpio, SpiDevice spi, Pin rstPin, Pin dcPin) {
     ...
     */
    public SSD1306(int width, int height, GpioController gpio, I2CBus i2c, int address, Pin rstPin) throws ReflectiveOperationException, IOException {
     ...
     * @see com.pi4j.io.spi.SpiFactory#getInstance(SpiChannel) SpiDevice factory
     */
    public SSD1306(int width, int height, GpioController gpio, SpiDevice spi, Pin dcPin) {
     ...
     * @see com.pi4j.io.i2c.I2CFactory#getInstance(int) I2C bus factory
     */
    public SSD1306(int width, int height, GpioController gpio, I2CBus i2c, int address) throws ReflectiveOperationException, IOException {
        this(width, height, gpio, i2c, address, null);
    }

	private SSD1306(int width, int height, boolean i2c, GpioController gpio, Pin rstPin) {
        this.width = width;
        this.height = height;
        this.pages = (height / 8);
	...
	public void scrollDiagonally(boolean left, int start, int end) {
		...
		this.command(left ? SSD1306_VERTICAL_AND_LEFT_HORIZONTAL_SCROLL
